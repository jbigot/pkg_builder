...
class DistInfo:
    def __init__(self, name:str, id:str, id_like:list=list()):
        self.name = name
        self.id = id
    ...
    def releases(self, id:str=None, codename:str=None, cpe:str=None, suite:str=None, release_date:date=None, eol_date:date=None, released:bool=None, supported:bool=None) -> List[ReleaseInfo]:
        def __filter_func(rel:DistInfo):
            if id is not None and rel.id != id:
                return False
            if codename is not None and rel.codename != codename:
                return False
            if cpe is not None and rel.cpe != cpe:
                return False
            if suite is not None and rel.suite != suite:
                return False
            if release_date is not None and rel.release_date != release_date:
                return False
            if eol_date is not None and rel.eol_date != eol_date:
                return False
            if released is not None and rel.released() != released:
                return False
            if supported is not None and rel.supported() != supported:
                return False
            return True
...
def distributions(id:str=None, id_like:set=None) -> Iterator[DistInfo]:
    def __filter_func(dist:DistInfo):
        if id is not None and dist.id != id:
            return False
        if id_like is not None and not set(id_like) <= set(dist.id_like+[dist.id]):
            return False
        return True
    return filter(__filter_func, __distribs)

def __debian_ubuntu_releases(distribution:DistInfo):
    with urlopen('https://salsa.debian.org/debian/distro-info-data/-/raw/master/'+distribution.id+'.csv') as rel_data:
        ...
        next(csv_data) # skip the title line: version,codename,series,created,release,eol,eol-server
        for order, rel in enumerate(csv_data):
            release = ReleaseInfo(distribution,
                                    rel[1] if rel[0] == '' else rel[0]+' ('+rel[1]+')',
                                    re.sub(r'[^0-9.].*', '', rel[0]),
                                    order,
                                    rel[2])
            if len(rel) > 4:
...
def __init_distribs():
    ...
    if __distribs is not None:
        return
    __distribs = set()
    debian = DistInfo("Debian GNU/Linux", 'debian')
    __debian_ubuntu_releases(debian)
    ...
    ubuntu = DistInfo("Ubuntu", 'ubuntu', id_like=['debian'])
    __debian_ubuntu_releases(ubuntu)
    ubuntu.releases(released=False)[0].suite = 'devel'
